...
use raster_runtime_exceptions::{DOMException, DOMExceptionName};
use raster_runtime_utils::mc_oneshot;
use rquickjs::{
    atom::PredefinedAtom,
    class::{Trace, Tracer},
    function::OnceFn,
...
impl<'js> AbortSignal<'js> {
    ...
    pub fn timeout(ctx: Ctx<'js>, milliseconds: u64) -> Result<Class<'js, Self>> {
        ...
        {
            raster_runtime_timers::set_timeout_interval(
                &ctx,
                cb,
                milliseconds,
                raster_runtime_utils::provider::ProviderType::Timeout,
            )?;
        }
        #[cfg(all(not(feature = "sleep-timers"), feature = "sleep-tokio"))]
        {
            use raster_runtime_utils::ctx::CtxExtension;
...
mod tests {
    ...
    async fn test_abort_signal() {
        test_async_with(|ctx| {
            ...
            raster_runtime_timers::init(&ctx).unwrap();
            Box::pin(async move {
