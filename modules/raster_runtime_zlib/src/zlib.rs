...
fn zlib_converter<'js>(
    ...
    let mut level = raster_runtime_compression::zlib::Compression::default();
    if let Some(options) = options.0 {
        if let Some(opt) = options.get_optional("level")? {
            level = raster_runtime_compression::zlib::Compression::new(opt);
        }
    }
    ...
    let _ = match command {
        ZlibCommand::Deflate => {
            raster_runtime_compression::zlib::encoder(src, level).read_to_end(&mut dst)?
        },
        ZlibCommand::DeflateRaw => {
            raster_runtime_compression::deflate::encoder(src, level).read_to_end(&mut dst)?
        },
        ZlibCommand::Gzip => {
            raster_runtime_compression::gz::encoder(src, level).read_to_end(&mut dst)?
        },
        ZlibCommand::Inflate => {
            raster_runtime_compression::zlib::decoder(src).read_to_end(&mut dst)?
        },
        ZlibCommand::InflateRaw => {
            raster_runtime_compression::deflate::decoder(src).read_to_end(&mut dst)?
        },
        ZlibCommand::Gunzip => {
            raster_runtime_compression::gz::decoder(src).read_to_end(&mut dst)?
        },
    };
