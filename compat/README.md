...
| --- | --- | --- | --- |
| Next App Router build | Next 16.2.10, React 19.2.5 | `next build` | Observing: past require-hook / `util.promisify` / `dns/promises` / Node semver / `process.on` / `fs.existsSync`. Regenerated `compat/next/compat.log` shows Raster exiting `1` with `UnhandledPromiseRejection: Error resolving module '../cli/next-build.js' from 'eval_script'`. SWC/native bindings remain out of scope. |
