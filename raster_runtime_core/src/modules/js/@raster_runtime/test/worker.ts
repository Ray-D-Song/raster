...
// Usage example
const {
  __RASTER_RUNTIME_TEST_SERVER_PORT: serverPortEnv,
  __RASTER_RUNTIME_TEST_WORKER_ID: workerIdEnv,
  __RASTER_RUNTIME_TEST_FILE: testFile,
} = process.env;
...
if (isNaN(workerId) || isNaN(serverPort) || !testFile) {
  throw new Error(
    "Test worker requires __RASTER_RUNTIME_TEST_SERVER_PORT, __RASTER_RUNTIME_TEST_WORKER_ID & __RASTER_RUNTIME_TEST_FILE env"
  );
}
