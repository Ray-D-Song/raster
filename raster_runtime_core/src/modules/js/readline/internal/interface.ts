// Ported from Node.js v24.3.0 lib/internal/readline/interface.js (MIT).
...
import { AbortError, codes } from './errors';
const { ERR_INVALID_ARG_VALUE, ERR_USE_AFTER_CLOSE } = codes;

import { validateAbortSignal, validateString, validateUint32 } from './validators';
import { assignFunctionName, kEmptyObject, getStringWidth, inspect, addAbortListener } from './util_helpers';
...
import { kSawKeyPress } from './symbols';
import { emitKeypressEvents as emitKeypressEventsFn } from './emitKeypressEvents';
...
function InterfaceConstructor(input, output, completer, terminal) {
  ...
  if (!this.terminal) {
    ...
    emitKeypressEvents = emitKeypressEventsFn;
...
class Interface extends InterfaceConstructor {
  ...
  [SymbolAsyncIterator]() {
    if (this[kLineObjectStream] === undefined) {
      // EventEmitter.on always yields args arrays; unwrap to a string line
      // (Node uses kFirstEventParam for the same effect).
      const EE = require("events");
      const source = EE.on(this, "line", {
        close: ["close"],
        highWaterMark: 1024,
        lowWaterMark: 1,
      });
      this[kLineObjectStream] = {
        ...
        async next() {
          ...
          return result.done ? result : { value: result.value[0], done: false };
        },
        return() {
