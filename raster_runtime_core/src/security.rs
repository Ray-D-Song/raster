...
use crate::environment::{ENV_RASTER_RUNTIME_NET_ALLOW, ENV_RASTER_RUNTIME_NET_DENY};
use crate::modules::{fetch, net};

pub fn init() -> StdResult<(), Box<dyn std::error::Error + Send + Sync>> {
    if let Ok(env_value) = env::var(ENV_RASTER_RUNTIME_NET_ALLOW) {
    ...
    if let Ok(env_value) = env::var(ENV_RASTER_RUNTIME_NET_DENY) {
