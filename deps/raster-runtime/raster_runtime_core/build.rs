...
fn generate_bytecode_cache(out_dir: &str) -> StdResult<(), Box<dyn Error>> {
    ...
    ctx.with(|ctx| {
        for dir_ent in WalkDir::new(BUNDLE_JS_DIR).into_iter().flatten() {
            ...
            let module_name = if !path_str.starts_with("raster_runtime-") {
                path.with_extension("")
                    .to_string_lossy()
                    .replace('\\', "/")
                    .replace("@raster_runtime/", "raster_runtime:")
            } else {
...
fn compress_bytecode(dictionary_path: String, source_files: Vec<String>) -> io::Result<usize> {
    ...
    let use_dictionary = fs::metadata(&dictionary_path)
    ...
    for filename in source_files {
        ...
        command.args(["--ultra", "-22", "-f"]);
        if use_dictionary {
            command.args(["-D", &dictionary_path]);
        }
        let output = command.args([&tmp_filename, "-o", &filename]).output()?;
...
fn generate_compression_dictionary(
    ...
    if file_count < 5 {
        info!("Too few samples for dictionary training, falling back to plain zstd compression");
        fs::write(dictionary_path, [])?;
        return Ok(());
    }
    ...
    if dictionary_filenames.len() < 5 {
        info!("Not enough eligible samples after filtering, skipping dictionary training");
        fs::write(dictionary_path, [])?;
        return Ok(());
    }
