...
fn build_pool_idle_timeout() -> Option<u64> {
    let Ok(env_value) = env::var(environment::ENV_RASTER_RUNTIME_NET_POOL_IDLE_TIMEOUT) else {
        return None;
    };
    ...
    if pool_idle_timeout > 300 {
        warn!(
            ...
            environment::ENV_RASTER_RUNTIME_NET_POOL_IDLE_TIMEOUT
        )
    }
...
fn build_extra_ca_certs() -> StdResult<Option<Vec<CertificateDer<'static>>>, io::Error> {
    if let Ok(extra_ca_certs) = env::var(environment::ENV_RASTER_RUNTIME_EXTRA_CA_CERTS) {
...
fn build_tls_versions() -> Vec<&'static SupportedProtocolVersion> {
    match env::var(environment::ENV_RASTER_RUNTIME_TLS_VERSION).as_deref() {
...
fn build_http_version() -> HttpVersion {
    match env::var(environment::ENV_RASTER_RUNTIME_HTTP_VERSION).as_deref() {
