...
// @ts-ignore
import { dimensions } from "raster_runtime:util";
...
class TestServer {
  ...
  private spawnWorker(id: number, file: string) {
    ...
    let env: any = {
      ...process.env,
      __RASTER_RUNTIME_TEST_SERVER_PORT: (this.server?.address() as any).port,
      __RASTER_RUNTIME_TEST_WORKER_ID: id.toString(),
      __RASTER_RUNTIME_TEST_FILE: file,
    };
    delete env.RASTER_RUNTIME_LOG;
    const proc = spawn(
      process.argv0,
      ["-e", `import("raster_runtime:test/worker").catch(console.error)`],
      {
        env,
      }
    );
