...
fn zstd_converter<'js>(
    ...
    let mut level = raster_runtime_compression::zstd::DEFAULT_COMPRESSION_LEVEL;
    ...
    let _ = match command {
        ZstdCommand::Compress => {
            raster_runtime_compression::zstd::encoder(src, level)?.read_to_end(&mut dst)?
        },
        ZstdCommand::Decompress => {
            raster_runtime_compression::zstd::decoder(src)?.read_to_end(&mut dst)?
        },
    };
