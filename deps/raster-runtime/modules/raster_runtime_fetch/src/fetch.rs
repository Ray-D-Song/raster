...
use raster_runtime_http::Agent;
use raster_runtime_http::HyperClient;
use raster_runtime_stream_web::ReadableStream;
...
fn apply_default_headers(
    ...
    if !detected_headers.contains("user-agent") {
        *req = std::mem::take(req).header("user-agent", ["raster_runtime ", VERSION].concat());
    }
...
mod tests {
    ...
    use raster_runtime_http::HttpsModule;
    ...
    use raster_runtime_test::{call_test, ModuleEvaluator};
    ...
    async fn test_fetch_function() {
        ...
        raster_runtime_compression::zstd::encoder(welcome_message.as_bytes(), 3)
            .unwrap()
        ...
        let mut data: Vec<u8> = Vec::new();
        raster_runtime_compression::brotli::encoder(welcome_message.as_bytes())
            .read_to_end(&mut data)
            .unwrap();
        ...
        let mut data: Vec<u8> = Vec::new();
        raster_runtime_compression::zlib::encoder(
            welcome_message.as_bytes(),
            raster_runtime_compression::zlib::Compression::default(),
        )
        .read_to_end(&mut data)
        .unwrap();
    ...
    async fn test_fetch_tls() {
        let mock_server = raster_runtime_test_tls::MockServer::start().await.unwrap();
        ...
        test_async_with(|ctx| {
            Box::pin(async move {
                ...
                assert_eq!(result, "Hello, raster_runtime!");
            })
        })
        .await;
    }
