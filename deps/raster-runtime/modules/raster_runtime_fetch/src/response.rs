...
use raster_runtime_abort::AbortSignal;
use raster_runtime_compression::streaming::StreamingDecoder;
use raster_runtime_context::CtxExtension;
use raster_runtime_json::{parse::json_parse, stringify::json_stringify};
use raster_runtime_stream_web::{
...
use raster_runtime_url::{url_class::URL, url_search_params::URLSearchParams};
use raster_runtime_utils::{bytes::ObjectBytes, mc_oneshot};
use rquickjs::{
    atom::PredefinedAtom,
    class::{Trace, Tracer},
    function::Opt,
    ...
    pub(crate) fn clone(&self, ctx: Ctx<'js>) -> Result<Self> {
        ...
        let cloned_body = match &*body {
            BodyVariant::Incoming(incoming, content_encoding) => {
                ...
                let (branch1, branch2) =
                    raster_runtime_stream_web::tee_readable_stream(ctx.clone(), stream.clone())?;
            ...
            BodyVariant::Provided(provided) => {
                ...
                if let Some(stream) = provided
                    ...
                    .and_then(Class::<ReadableStream>::from_object)
                {
                    let (branch1, branch2) = raster_runtime_stream_web::tee_readable_stream(
                        ctx.clone(),
                        stream.clone(),
                    )?;
...
fn create_body_stream<'js>(
    ...
    // Set native_pull on controller for reader.read() fast-path
    {
        ...
        let np: Rc<raster_runtime_stream_web::NativePullFn> =
            Rc::new(move |ctx: &rquickjs::Ctx<'js>| {
                let mut guard = incoming2.write().unwrap();
                let Some(body) = guard.as_mut() else {
                    return Ok(raster_runtime_stream_web::NativePullResult::Eof);
                };
                    ...
                    match Pin::new(&mut *body).poll_frame(&mut poll_cx) {
                        Poll::Ready(Some(Ok(frame))) => {
                            ...
                            let val = match chunk {
                                ...
                                None => TypedArray::<u8>::new_copy(ctx.clone(), &first_data)?
                                    .into_value(),
                            };
                                ...
                                if let Some(dec) = state2.write().unwrap().take() {
                                    match dec.finish() {
                                        Ok(remaining) if !remaining.is_empty() => {
                                            let val =
                                                TypedArray::<u8>::new(ctx.clone(), remaining)?
                                                    .into_value();
                                            return Ok(NativePullResult::Ready(val));
                                        },
                                        Err(e) => {
        ...
        if let ReadableStreamControllerClass::ReadableStreamDefaultController(ref ctrl) =
            s.controller
        {
            ctrl.borrow_mut().native_pull = Some(raster_runtime_stream_web::NativePull(np));
        }
    }
...
#[cfg(test)]
mod tests {
    use raster_runtime_test::{test_async_with_opts, TestOptions};
    ...
    async fn test_response_stream() {
        ...
        let welcome_message = "Hello, raster_runtime!";
    ...
    async fn test_response_clone_error() {
        ...
        test_async_with_opts(
            |ctx| {
                raster_runtime_stream_web::init(&ctx).unwrap();
                crate::init(&ctx).unwrap();
                Box::pin(async move {
                    let globals = ctx.globals();
                    let run = async {
    ...
    async fn test_response_compressed_body_stream() {
        ...
        raster_runtime_compression::gz::encoder(
            message.as_bytes(),
            raster_runtime_compression::gz::Compression::default(),
        )
